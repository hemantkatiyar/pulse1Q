...
def calc_uf_eg(u, nsegments, rfi, ssx, ssy, ssz, delT, nions, amp_max, deltaz):
    ...
    for rfindex in range(len(rfi)):
        for segment in range(nsegments):
            if segment == 0:
                Uf[rfindex][segment] = expm(-1.0j * delT * rfi[rfindex] * (deltaz * (ssz / 2) + ux[segment] * (ssx / 2) 
                                                                          +  uy[segment] * (ssy / 2)))
            else:
                Uf[rfindex][segment] = np.matmul(expm(-1.0j * delT * rfi[rfindex] * (deltaz * (ssz / 2) + ux[segment] * (ssx / 2) 
                                                                                    +  uy[segment] * (ssy / 2))),
...
def calc_fidelity_ed(u, nsegments, rfi, ssx, ssy, ssz, Ut, delT, nions, amp_max, deltaz):
...
def calc_fidelity_ed_noisy(u, nsegments, rfi, ssx, ssy, ssz, Ut, delT, nions, amp_max):
    ...
    for rfindex in range(len(rfi)):
        for segment in range(nsegments):
            if segment == 0:
                Uf[rfindex][segment] = expm(-1.0j * delT * rfi[rfindex] * (deltaz[segment] * (ssz / 2) + ux[segment] * (ssx / 2) 
                                                                          +  uy[segment] * (ssy / 2)))
            else:
                Uf[rfindex][segment] = np.matmul(expm(-1.0j * delT * rfi[rfindex] * (deltaz[segment] * (ssz / 2) + ux[segment] * (ssx / 2) 
                                                                                    +  uy[segment] * (ssy / 2))),
                                                                                       Uf[rfindex][segment-1])
    Fid = 0
